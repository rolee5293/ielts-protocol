...
test.describe("云同步", () => {
  test("上传只写本设备专属行，历史行原样保留", async ({ page, cloud }) => {
    ...
    cloud.rows = [{ app: "ielts", data: legacy, updated_at: OLD }];
    ...
    expect(cloud.lastRowKey()).toMatch(/^ielts#.+/);
    expect(cloud.lastRowKey()).not.toBe("ielts");

    // 历史行未被改写
    const stillLegacy = cloud.rows.find((r) => r.app === "ielts");
  ...
  test("同一设备重复上传复用同一行，不会每次新建", async ({ page, cloud }) => {
    ...
    expect(cloud.rows.filter((r) => r.app.startsWith("ielts#"))).toHaveLength(1);
  });
  ...
  test("多设备存档合并后取进度最快的一方，累计量不翻倍", async ({ page, cloud }) => {
    ...
    cloud.rows = [
      { app: "ielts#deviceA", data: devA, updated_at: "2026-08-08T10:00:00+00:00" },
      { app: "ielts#deviceB", data: devB, updated_at: "2026-08-08T11:00:00+00:00" },
    ];
  ...
  test("本机已有进度时与云端取并，本地进度不被云端覆盖", async ({ page, cloud }) => {
    ...
    cloud.rows = [{ app: "ielts#other", data: savedProgress({ xp: 400, wordCursor: 50 }), updated_at: OLD }];
  ...
  test("首次下载未返回前不上传，避免本地旧存档冲掉云端", async ({ page, cloud }) => {
    ...
    cloud.rows = [{ app: "ielts#rich", data: savedProgress({ xp: 5000, wordCursor: 200 }), updated_at: OLD }];
  ...
  test("下载超时也只写本设备行，不会波及其他设备的存档", async ({ page, cloud }) => {
    ...
    cloud.rows = [{ app: "ielts#other", data: savedProgress({ xp: 5000 }), updated_at: OLD }];
    ...
    expect(cloud.lastRowKey()).toMatch(/^ielts#.+/);
    expect(cloud.lastRowKey()).not.toBe("ielts#other");
    expect((cloud.rows.find((r) => r.app === "ielts#other")?.data as { xp: number }).xp).toBe(5000);
  });
